...
export const GlmSettings = Schema.Struct({
	...
	baseUrl: TrimmedString.pipe(
		Schema.withDecodingDefault(
			Effect.succeed("https://api.z.ai/api/anthropic"),
		),
	),
	customModels: Schema.Array(Schema.String).pipe(
		Schema.withDecodingDefault(Effect.succeed([])),
...
export const ServerSettings = Schema.Struct({
	...
	providers: Schema.Struct({
		...
		glmClaudeAgent: GlmSettings.pipe(Schema.withDecodingDefault(Effect.succeed({}))),
	}).pipe(Schema.withDecodingDefault(Effect.succeed({}))),
...
const ModelSelectionPatch = Schema.Union([
	...
	Schema.Struct({
		provider: Schema.optionalKey(Schema.Literal("glmClaudeAgent")),
		model: Schema.optionalKey(TrimmedNonEmptyString),
		options: Schema.optionalKey(ClaudeModelOptionsPatch),
	}),
]);
...
export const ServerSettingsPatch = Schema.Struct({
	...
	providers: Schema.optionalKey(
		Schema.Struct({
			...
			glmClaudeAgent: Schema.optionalKey(GlmSettingsPatch),
		}),
	),
});
