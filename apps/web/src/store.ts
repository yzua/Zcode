...
function normalizeModelSelection<
	T extends {
		provider: "codex" | "claudeAgent" | "glmClaudeAgent";
		model: string;
	},
>(selection: T): T {
	return {
...
function threadTurnStatesEqual(
	...
	return (
		...
		sourceProposedPlansEqual(
			left.pendingSourceProposedPlan,
			right.pendingSourceProposedPlan,
		)
	);
}
...
function ensureThreadRegistered(
	...
	if (previousProjectId !== nextProjectId) {
		...
		if (previousProjectId) {
			const previousIds =
				threadIdsByProjectId[previousProjectId] ?? EMPTY_THREAD_IDS;
		...
		const projectThreadIds =
			threadIdsByProjectId[nextProjectId] ?? EMPTY_THREAD_IDS;
...
function writeThreadState(
	...
	if (
		!threadSessionsEqual(previousThread?.session ?? null, nextThread.session)
	) {
		nextState = {
			...nextState,
			threadSessionById: {
...
function writeThreadShellState(
	...
	if (
		!threadSessionsEqual(
			state.threadSessionById[nextThread.shell.id] ?? null,
			nextThread.session,
		)
	) {
		nextState = {
	...
	if (
		!threadTurnStatesEqual(
			state.threadTurnStateById[nextThread.shell.id],
...
function retainThreadScopedRecord<T>(
	...
	return Object.fromEntries(
		Object.entries(record).flatMap(([threadId, value]) =>
			nextThreadIds.has(threadId as ThreadId)
				? [[threadId, value] as const]
				: [],
		),
	) as Record<ThreadId, T>;
}

function removeThreadState(
	state: EnvironmentState,
	threadId: ThreadId,
): EnvironmentState {
	...
	const nextThreadIdsByProjectId =
		nextProjectThreadIds.length === 0
			? (() => {
					const { [shell.projectId]: _removed, ...rest } =
	...
	const { [threadId]: _removedTurnDiffIds, ...turnDiffIdsByThreadId } =
...
function syncEnvironmentShellSnapshot(
	...
	const nextProjects = snapshot.projects.map((project) =>
		mapProject(project, environmentId),
	);
	...
	switch (event.type) {
		case "project.created": {
			...
			if (existingProjectId !== null && existingProjectId !== nextProject.id) {
				const { [existingProjectId]: _removedProject, ...restProjectById } =
					state.projectById;
				projectById = {
					...restProjectById,
				...
				projectIds =
					...
					!state.projectIds.includes(nextProject.id)
		...
		case "project.deleted": {
			...
			const { [event.payload.projectId]: _removedProject, ...projectById } =
				state.projectById;
			return {
				...state,
				projectById,
...
function applyEnvironmentShellEvent(
	...
	switch (event.kind) {
		...
		case "project-removed": {
			...
			const { [event.projectId]: _removedProject, ...projectById } =
				state.projectById;
			return {
				...state,
				projectById,
...
export function applyOrchestrationEvents(
	...
	const nextEnvironmentState = events.reduce(
		(nextState, event) =>
			applyEnvironmentOrchestrationEvent(nextState, event, environmentId),
...
export function selectThreadShellsAcrossEnvironments(
	state: AppState,
): ThreadShell[] {
	return getEnvironmentEntries(state).flatMap(([, environmentState]) =>
...
export function selectSidebarThreadsAcrossEnvironments(
	...
	return getEnvironmentEntries(state).flatMap(
...
export function selectSidebarThreadsForProjectRefs(
	...
	if (refs.length === 1)
		return selectSidebarThreadsForProjectRef(state, refs[0]);
	return refs.flatMap((ref) => selectSidebarThreadsForProjectRef(state, ref));
}

export function selectBootstrapCompleteForActiveEnvironment(
	state: AppState,
): boolean {
	return selectEnvironmentState(state, state.activeEnvironmentId)
		.bootstrapComplete;
}
...
export function selectProjectByRef(
	...
	return ref
		? selectEnvironmentState(state, ref.environmentId).projectById[
				ref.projectId
			]
		: undefined;
}
...
export function selectThreadExistsByRef(
	...
	return ref
		? selectEnvironmentState(state, ref.environmentId).threadShellById[
				ref.threadId
			] !== undefined
		: false;
}
...
export function selectSidebarThreadSummaryByRef(
	...
	return ref
		? selectEnvironmentState(state, ref.environmentId).sidebarThreadSummaryById[
				ref.threadId
			]
		: undefined;
}
...
export function selectThreadIdsByProjectRef(
	...
	return ref
		? (selectEnvironmentState(state, ref.environmentId).threadIdsByProjectId[
				ref.projectId
			] ?? EMPTY_THREAD_IDS)
		: EMPTY_THREAD_IDS;
}
...
export function setThreadBranch(
	...
	const nextEnvironmentState = updateThreadState(
		...
		threadRef.threadId,
		(thread) => {
			if (thread.branch === branch && thread.worktreePath === worktreePath)
				return thread;
