...
export const WORKFLOW_PROMPTS: readonly WorkflowPrompt[] = [
	{
		...
		label: "Commit Split",
		shortLabel: "CM",
		description: "Split changes into logical commits",
		icon: GitCommitHorizontalIcon,
		prompt: `Objective: transform the current working tree into the smallest useful sequence of logical commits with zero unrelated changes.
...
- Detect the actual stack and workflow from source-of-truth files instead of guessing: justfile, Makefile, flake.nix, package.json, pnpm-workspace.yaml, pyproject.toml, requirements files, Cargo.toml, go.mod, Dockerfile, compose file, terraform files, CI workflows, and top-level docs.
...
- End with the remaining git status and any intentionally uncommitted work.`,
	...
	{
		id: "refactorMaintainability",
		label: "Refactor",
		shortLabel: "RF",
		description: "Improve maintainability with zero behavior drift",
		icon: RefreshCwIcon,
		prompt: `Goal: improve maintainability and clarity with zero unintended behavior drift.
...
- Map each applied change to a specific maintainability gain such as clearer ownership, reduced duplication, smaller surface area, or cleaner boundaries.`,
	},
	{
		...
		label: "Bug Fix",
		shortLabel: "FX",
		description: "Root cause bug fix with evidence",
		icon: BugIcon,
		prompt: `Resolve a bug or regression by proving the root cause before applying a fix.
...
- End with what is verified, what remains inferred, residual risk, unverified edges, and any follow-up hardening that was intentionally deferred.`,
	},
	{
		...
		shortLabel: "SA",
		description: "Security audit with evidence",
		icon: ShieldIcon,
		prompt: `Run a security audit with evidence only.
...
- End with a prioritized remediation backlog split into immediate fixes, short-term hardening, and structural follow-ups.`,
	...
	{
		id: "dependencyUpgrade",
		label: "Dependency Upgrade",
		shortLabel: "DU",
		description: "Upgrade dependencies with minimal risk",
		icon: RocketIcon,
		prompt: `Upgrade a dependency, package, plugin, tool, runtime, or provider with minimal risk and explicit compatibility handling.
...
- If the upgrade was blocked, report the blocker with the exact file, command, release note, migration gap, or compatibility issue that stopped progress.`,
	},
	{
		...
		label: "Build Perf",
		shortLabel: "BP",
		description: "Build and delivery performance pass",
		icon: HourglassIcon,
		prompt: `Run a build and delivery performance pass with measurable evidence.
...
- If no trustworthy measurement path exists, state that limitation clearly and switch to evidence-backed static analysis instead of fake numbers.`,
	...
	{
		id: "runtimePerformance",
		label: "Runtime Perf",
		shortLabel: "RP",
		description: "Runtime code performance pass",
		icon: GaugeIcon,
		prompt: `Run a code-performance pass with measurable evidence.
...
- If no trustworthy measurement path exists, state that limitation clearly and switch to evidence-backed static analysis instead of fake numbers.`,
	},
	{
		...
		label: "Docs Sync",
		shortLabel: "MD",
		description: "Sync markdown docs with codebase",
		icon: FileTextIcon,
		prompt: `Synchronize markdown and operator-facing docs with current repository reality.
...
export const WORKFLOW_PROMPT_MAP: Readonly<Record<string, WorkflowPrompt>> =
	Object.fromEntries(WORKFLOW_PROMPTS.map((p) => [p.id, p]));

export const DEFAULT_WORKFLOW_PROMPT_ICON = ScrollTextIcon;
