...
const terminalContextIdListsEqual = (
	...
	contexts.every((context, index) => context.id === ids[index]);

const ComposerFooterModeControls = memo(
	function ComposerFooterModeControls(props: {
...
const ComposerFooterPrimaryActions = memo(
...
export const ChatComposer = memo(
	forwardRef<ChatComposerHandle, ChatComposerProps>(
		function ChatComposer(props, ref) {
			...
			const threadProvider =
				activeThreadModelSelection?.provider ??
				activeProjectDefaultModelSelection?.provider ??
				null;
			...
			const { modelOptions: composerModelOptions, selectedModel } =
				useEffectiveComposerModelState({
			...
			>(
				() => ({
					codex:
						providerStatuses.find((provider) => provider.provider === "codex")
							?.models ?? [],
					claudeAgent:
						providerStatuses.find(
			...
			const [composerHighlightedSearchKey, setComposerHighlightedSearchKey] =
				useState<string | null>(null);
			const [isDragOverComposer, setIsDragOverComposer] = useState(false);
			const [isComposerFooterCompact, setIsComposerFooterCompact] =
				useState(false);
			const [
			...
			const activeComposerMenuItemRef = useRef<ComposerCommandItem | null>(
				null,
			);
			...
			const [debouncedPathQuery, composerPathQueryDebouncer] =
				useDebouncedValue(
					pathTriggerQuery,
			...
			const activeComposerMenuItem = useMemo(() => {
				...
				return (
					composerMenuItems.find((item) => item.id === activeItemId) ?? null
				);
			}, [
			...
			const setPromptFromTraits = useCallback(
				(nextPrompt: string) => {
					...
					setComposerTrigger(
						detectComposerTrigger(nextPrompt, nextPrompt.length),
					);
					scheduleComposerFocus();
				},
				[
			...
			const removeComposerTerminalContextFromDraft = useCallback(
				(contextId: string) => {
					...
					const removal = removeInlineTerminalContextPlaceholder(
						promptRef.current,
						contextIndex,
					);
					...
					setComposerCursor(nextCursor);
					setComposerTrigger(
						detectComposerTrigger(removal.prompt, removal.cursor),
					);
				},
				[
					composerDraftTarget,
			...
			useEffect(() => {
				...
				setComposerCursor((existing) =>
					clampCollapsedComposerCursor(prompt, existing),
				);
			}, [prompt, promptRef]);
			...
			useEffect(() => {
				...
				const nextQuestionId =
					activePendingProgress?.activeQuestion?.id ?? null;
			...
			useLayoutEffect(() => {
				...
				const measureFooterCompactness = () => {
					...
					const footerCompact = shouldUseCompactComposerFooter(
						composerFormWidth,
						{
				...
				const observer = new ResizeObserver((entries) => {
					...
					setIsComposerFooterCompact((previous) =>
						previous === nextCompactness.footerCompact
							? previous
				...
				void (async () => {
					...
					try {
						const currentPersistedAttachments =
							getPersistedAttachmentsForThread();
			...
			const applyPromptReplacement = useCallback(
				(
					...
					const safeStart = Math.max(
						0,
						Math.min(currentText.length, rangeStart),
					);
			...
			const readComposerSnapshot = useCallback((): {
				...
				return {
					...
					terminalContextIds: composerTerminalContexts.map(
						(context) => context.id,
					),
				};
				...
				(item: ComposerCommandItem) => {
					...
					if (item.type === "path") {
						...
						const applied = applyPromptReplacement(
							...
							replacement,
							{
								expectedText: snapshot.value.slice(
									trigger.rangeStart,
									replacementRangeEnd,
								),
							},
			...
			useImperativeHandle(
				...
				() => ({
					...
					addTerminalContext: (selection: TerminalContextSelection) => {
						...
						setComposerTrigger(
							detectComposerTrigger(insertion.prompt, insertion.cursor),
						);
						window.requestAnimationFrame(() => {
