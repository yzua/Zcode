...
const PROVIDER_SETTINGS: readonly InstallProviderSettings[] = [
	...
	},
	{
		...
		title: "Claude (GLM)",
		binaryPlaceholder: "API Key",
		binaryDescription: "Z.AI API key for GLM access",
	},
] as const;
...
export function GeneralSettingsPanel() {
	...
	>({
		...
		glmClaudeAgent: Boolean(
			settings.providers.glmClaudeAgent.apiKey !== "" ||
				settings.providers.glmClaudeAgent.baseUrl !==
					DEFAULT_UNIFIED_SETTINGS.providers.glmClaudeAgent.baseUrl ||
				settings.providers.glmClaudeAgent.customModels.length > 0,
		),
	});
	...
	const providerCards = PROVIDER_SETTINGS.map((providerSettings) => {
		...
		return {
			...
			binaryPathValue:
				"binaryPath" in providerConfig ? providerConfig.binaryPath : undefined,
							...
							>
								<CollapsibleContent>
									<div className="space-y-0">
										{providerCard.provider === "glmClaudeAgent" ? (
											<>
												<div className="border-t border-border/60 px-4 py-3 sm:px-5">
													<label
														htmlFor="provider-install-glm-api-key"
														className="block"
													>
														...
														<Input
															id="provider-install-glm-api-key"
															className="mt-1.5"
															type="password"
															value={settings.providers.glmClaudeAgent.apiKey}
															onChange={(event) =>
																updateSettings({
																	providers: {
																		...settings.providers,
																		glmClaudeAgent: {
																			...settings.providers.glmClaudeAgent,
														...
														<span className="mt-1 block text-xs text-muted-foreground">
															Z.AI API key for GLM access
														</span>
													</label>
												</div>
												<div className="border-t border-border/60 px-4 py-3 sm:px-5">
													<label
														htmlFor="provider-install-glm-base-url"
														className="block"
													>
														<span className="text-xs font-medium text-foreground">
															Base URL
														...
														<Input
															id="provider-install-glm-base-url"
															className="mt-1.5"
															value={settings.providers.glmClaudeAgent.baseUrl}
															onChange={(event) =>
																updateSettings({
																	providers: {
																		...settings.providers,
										...
										{providerCard.provider === "claudeAgent" ? (
											<div className="border-t border-border/60 px-4 py-3 sm:px-5">
												...
												>
													...
													/>
													<span className="mt-1 block text-xs text-muted-foreground">
														Additional CLI arguments passed to Claude Code on
														session start.
													</span>
												</label>
											</div>
