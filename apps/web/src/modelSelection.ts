...
> = {
	...
	glmClaudeAgent: {
		...
		title: "Claude (GLM)",
		description:
			"Save additional GLM model slugs for the picker and `/model` command.",
		placeholder: "your-glm-model-slug",
		example: "glm-5.1",
	},
};

export const MODEL_PROVIDER_SETTINGS = Object.values(
...
export function getAppModelOptions(
	...
	const selectedModelMatchesExistingName =
		...
		options.some(
			(option) => option.name.toLowerCase() === trimmedSelectedModel,
		);
	if (
...
export function getCustomModelOptionsByProvider(
	...
	return {
		...
		glmClaudeAgent: getAppModelOptions(
			settings,
			providers,
			"glmClaudeAgent",
			selectedProvider === "glmClaudeAgent" ? selectedModel : undefined,
		),
	};
}
...
export function resolveAppModelSelectionState(
	...
	const { modelOptionsForDispatch } = getComposerProviderState({
		...
		modelOptions: {
			[provider]:
				provider === selection.provider ? selection.options : undefined,
		},
	});
