/**
 * GlmAdapterLive - Scoped live implementation for the GLM (Z.AI) provider adapter.
 *
...
import {
	GlmAdapter,
	type GlmAdapterShape,
} from "../Services/GlmAdapter.ts";
...
function normalizeClaudeTokenUsage(
	...
	const totalProcessedTokens =
		...
		Number.isFinite(usage.total_tokens)
			? usage.total_tokens
			: undefined) ??
		(derivedTotalProcessedTokens > 0 ? derivedTotalProcessedTokens : undefined);
	...
	return {
		...
		Number.isFinite(usage.duration_ms)
...
function readClaudeResumeState(
	...
	const threadIdCandidate =
		typeof cursor.threadId === "string" ? cursor.threadId : undefined;
	const threadId =
	...
	const resumeCandidate =
		typeof cursor.resume === "string"
			...
			: typeof cursor.sessionId === "string"
				...
				: undefined;
	const resume =
		resumeCandidate && isUuid(resumeCandidate) ? resumeCandidate : undefined;
	const resumeSessionAt =
		typeof cursor.resumeSessionAt === "string"
			? cursor.resumeSessionAt
			: undefined;
	const turnCountValue =
		typeof cursor.turnCount === "number" ? cursor.turnCount : undefined;

	return {
		...(threadId ? { threadId } : {}),
		...(resume ? { resume } : {}),
		...(resumeSessionAt ? { resumeSessionAt } : {}),
		...(turnCountValue !== undefined &&
		Number.isInteger(turnCountValue) &&
		turnCountValue >= 0
...
function buildPromptText(input: ProviderSendTurnInput): string {
	const rawEffort =
		input.modelSelection?.provider === "glmClaudeAgent"
...
function buildUserMessage(input: {
	...
	return {
		...
		message: {
			role: "user",
			content:
				input.sdkContent as unknown as SDKUserMessage["message"]["content"],
		},
	} as SDKUserMessage;
}
...
const makeGlmAdapter = Effect.fn("makeGlmAdapter")(function* (
	...
	const snapshotThread = Effect.fn("snapshotThread")(function* (
		context: ClaudeSessionContext,
	) {
	...
	const ensureAssistantTextBlock = Effect.fn("ensureAssistantTextBlock")(
		function* (
			context: ClaudeSessionContext,
			blockIndex: number,
			options?: {
	...
	const createSyntheticAssistantTextBlock = Effect.fn(
	...
	const completeAssistantTextBlock = Effect.fn("completeAssistantTextBlock")(
		function* (
			context: ClaudeSessionContext,
			...
			yield* offerRuntimeEvent({
				...
				payload: {
					...
					...(block.fallbackText.length > 0
						? { detail: block.fallbackText }
						: {}),
				},
				providerRefs: nativeProviderRefs(context),
				...(options?.rawMethod || options?.rawPayload
	...
	const emitProposedPlanCompleted = Effect.fn("emitProposedPlanCompleted")(
		function* (
			context: ClaudeSessionContext,
			input: {
				readonly planMarkdown: string;
	...
	const handleStreamEvent = Effect.fn("handleStreamEvent")(function* (
		...
		if (event.type === "content_block_delta") {
			...
			if (event.delta.type === "input_json_delta") {
				...
				const partialInputJson =
					tool.partialInputJson + event.delta.partial_json;
		...
		if (event.type === "content_block_start") {
			...
			const inputFingerprint =
				Object.keys(toolInput).length > 0
					? toolInputFingerprint(toolInput)
					: undefined;
			...
			const tool: ToolInFlight = {
				...
				partialInputJson: "",
				...(inputFingerprint
					? { lastEmittedInputFingerprint: inputFingerprint }
					: {}),
			};
	...
	const handleSdkTelemetryMessage = Effect.fn("handleSdkTelemetryMessage")(
		function* (context: ClaudeSessionContext, message: SDKMessage) {
			const stamp = yield* makeEventStamp();
			const base = {
				eventId: stamp.eventId,
	...
	const startSession: GlmAdapterShape["startSession"] = Effect.fn(
		"startSession",
	)(function* (input) {
		...
		const contextRef = yield* Ref.make<ClaudeSessionContext | undefined>(
			undefined,
		);
		...
		const handleAskUserQuestion = Effect.fn("handleAskUserQuestion")(function* (
			...
			const questions: Array<UserInputQuestion> = rawQuestions.map(
				(q: Record<string, unknown>, idx: number) => ({
					...
					header:
						typeof q.header === "string" ? q.header : `Question ${idx + 1}`,
			...
			const onAbort = () => {
				...
				runFork(
					Deferred.succeed(answersDeferred, {} as ProviderUserInputAnswers),
				);
			};
		...
		const glmSettings = allSettings.providers.glmClaudeAgent;
		const claudeSettings = allSettings.providers.claudeAgent;
		const claudeBinaryPath = claudeSettings.binaryPath;
		const extraArgs = parseCliArgs(claudeSettings.launchArgs).flags;
		const glmEnv = {
			...process.env,
			ANTHROPIC_AUTH_TOKEN: glmSettings.apiKey,
			ANTHROPIC_BASE_URL: glmSettings.baseUrl || "https://api.z.ai/api/anthropic",
		...
		const fastMode =
			modelSelection?.options?.fastMode === true && caps.supportsFastMode;
		const thinking =
	...
	const sendTurn: GlmAdapterShape["sendTurn"] = Effect.fn("sendTurn")(
		function* (input) {
				...
				if (context.currentApiModelId !== apiModelId) {
					yield* Effect.tryPromise({
						try: () => context.query.setModel(apiModelId),
							...
							toRequestError(input.threadId, "turn/setModel", cause),
					});
			...
			if (input.interactionMode === "plan") {
				yield* Effect.tryPromise({
					try: () => context.query.setPermissionMode("plan"),
						...
						toRequestError(input.threadId, "turn/setPermissionMode", cause),
				});
	...
	const interruptTurn: GlmAdapterShape["interruptTurn"] = Effect.fn(
		"interruptTurn",
	...
	const readThread: GlmAdapterShape["readThread"] = Effect.fn("readThread")(
		function* (threadId) {
			const context = yield* requireSession(threadId);
	...
	const rollbackThread: GlmAdapterShape["rollbackThread"] = Effect.fn(
		"rollbackThread",
	...
	const respondToRequest: GlmAdapterShape["respondToRequest"] = Effect.fn(
		"respondToRequest",
	...
	const respondToUserInput: GlmAdapterShape["respondToUserInput"] = Effect.fn(
		"respondToUserInput",
	...
	const stopSession: GlmAdapterShape["stopSession"] = Effect.fn("stopSession")(
		function* (threadId) {
			const context = yield* requireSession(threadId);
			yield* stopSessionInternal(context, {
				emitExitEvent: true,
