					...
					updateSettings: (patch) =>
						Ref.get(currentSettingsRef).pipe(
							...
							Effect.tap((nextSettings) =>
								Ref.set(currentSettingsRef, nextSettings),
							),
						),
...
const PROVIDER_ORDER: readonly ProviderKind[] = ["codex", "claudeAgent", "glmClaudeAgent"];
	...
	if (
		...
		for (const key of Object.keys(currentRecord)) {
			if (ATOMIC_SETTINGS_KEYS.has(key)) {
				...
				const stripped = stripDefaultServerSettings(
					currentRecord[key],
					defaultsRecord[key],
				);
...
const makeServerSettings = Effect.gen(function* () {
	...
	const writeSettingsAtomically = (settings: ServerSettings) => {
		...
		return Effect.succeed(`${JSON.stringify(sparseSettings, null, 2)}\n`).pipe(
			Effect.tap(() =>
				fs.makeDirectory(pathService.dirname(settingsPath), {
					recursive: true,
				}),
			),
	...
	const start = Effect.gen(function* () {
		const shouldStart = yield* Ref.modify(startedRef, (started) => [
			!started,
			true,
		]);
		if (!shouldStart) {
		...
		if (startupExit._tag === "Failure") {
			yield* Deferred.failCause(startedDeferred, startupExit.cause).pipe(
				Effect.orDie,
			);
